...
История чатов в PostgreSQL. Премиум-эмодзи Telegram во всех сообщениях и кнопках.
...
ENV (опционально):
    SMART_API_KEY  — ключ прокси (по умолчанию подставлен вшитый)
    SMART_BASE_URL — базовый URL (по умолчанию https://api.smartapi.shop)

Запуск:
    pip install -r requirements.txt
    BOT_TOKEN=... DATABASE_URL=postgres://user:pass@host:5432/db python bot.py
"""
...
def premoji(key: str) -> str:
    """HTML-тег для премиум-эмодзи. Юзеры без Premium увидят fallback."""
...
async def get_or_create_user(user_id: int, username: Optional[str]) -> dict:
    async with db_pool.acquire() as conn:  # type: ignore[union-attr]
        row = await conn.fetchrow(
            "SELECT user_id, model_key FROM users WHERE user_id = $1",
            user_id,
        )
        if row is None:
            await conn.execute(
                "INSERT INTO users (user_id, username, model_key) "
                "VALUES ($1, $2, $3) "
...
async def set_user_model(user_id: int, model_key: str) -> None:
    async with db_pool.acquire() as conn:  # type: ignore[union-attr]
        await conn.execute(
            "UPDATE users SET model_key = $1 WHERE user_id = $2",
            model_key, user_id,
        )
...
def main_keyboard() -> ReplyKeyboardMarkup:
    """Постоянная клавиатура снизу с премиум-иконками."""
...
def model_keyboard(current: str) -> InlineKeyboardMarkup:
    """Инлайн-выбор модели: текущая с галочкой, остальные с крестиком."""
...
async def typing_anim(
    ...
    """Раз в секунду правит сообщение, пока не взведён stop."""
    ...
    while not stop.is_set():
        ...
        try:
            ...
            # "Message is not modified" и т.п. — игнорим
            pass
        try:
...
# Ловим любые неизвестные команды: текст, начинающийся с "/".
# Регистрация ПОСЛЕ конкретных команд (CommandStart, Command("model") и т.д.),
# поэтому /start и /model не сюда не попадут.
...
# ──────── Reply-кнопки (должны быть ДО общего chat-хэндлера) ────────────────
...
async def handle_chat(message: Message, bot: Bot) -> None:
    ...
    # 2) Берём последние N сообщений (включая только что сохранённый вопрос)
    history = await get_history(user.id, MAX_HISTORY)

    # 3) Запускаем «Думаю… 0с» и аниматор
    ...
    # Параллельно дёрнем нативный «печатает…»
    ...
    # 5) Сохраняем ответ (только успешный)
    if not error_occurred:
        await save_message(user.id, "assistant", answer, model_name)

    # 6) Заменяем «Думаю…» на ответ (или шлём кусками)
    try:
