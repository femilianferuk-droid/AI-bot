...
• Клонирование через HTTP (zip-архив, без Git)
...
def clone_repo(url: str) -> Path:
    """Скачивает архив репозитория с GitHub и распаковывает."""
    url = url.rstrip("/").replace(" ", "")
    if not url.startswith("https://"):
        url = "https://" + url

    # Извлекаем user/repo
    url_path = url.replace("https://github.com/", "").replace("https://gitlab.com/", "").replace("https://bitbucket.org/", "")
    ...
    log.info(f"Скачиваю архив: {archive_url}")
    ...
    try:
        ...
        archive_url = f"https://github.com/{owner}/{repo_name}/archive/refs/heads/master.zip"
        req = urllib.request.Request(archive_url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=60) as response:
            data = response.read()
    ...
    # GitHub кладёт всё в папку repo-main или repo-master — переименовываем
    extracted = list(WORK_DIR.glob(f"{repo_name}-*"))
...
async def cmd_repo(message: Message) -> None:
    ...
    status = await message.answer(
        f"{premoji('loading')} Скачиваю репозиторий..."
    )
    try:
